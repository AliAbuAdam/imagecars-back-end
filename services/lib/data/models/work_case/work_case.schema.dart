...
class _WorkCaseRepository extends BaseRepository
  ...
  Future<List<int>> insert(List<WorkCaseInsertRequest> requests) async {
    ...
    var rows = await db.query(
      'INSERT INTO "work_cases" ( "title", "car_model", "service_id", "photos", "video", "description", "master_name", "work_houres", "price" )\n'
      'VALUES ${requests.map((r) => '( ${values.add(r.title)}:text, ${values.add(r.carModel)}:text, ${values.add(r.serviceId)}:int8, ${values.add(r.photos)}:_text, ${values.add(r.video)}:text, ${values.add(r.description)}:text, ${values.add(r.masterName)}:text, ${values.add(r.workHoures)}:float8, ${values.add(r.price)}:float8 )').join(', ')}\n'
      'RETURNING "id"',
      values.values,
    );
  ...
  Future<void> update(List<WorkCaseUpdateRequest> requests) async {
    ...
    await db.query(
      'UPDATE "work_cases"\n'
      'SET "title" = COALESCE(UPDATED."title", "work_cases"."title"), "car_model" = COALESCE(UPDATED."car_model", "work_cases"."car_model"), "service_id" = COALESCE(UPDATED."service_id", "work_cases"."service_id"), "photos" = COALESCE(UPDATED."photos", "work_cases"."photos"), "video" = COALESCE(UPDATED."video", "work_cases"."video"), "description" = COALESCE(UPDATED."description", "work_cases"."description"), "master_name" = COALESCE(UPDATED."master_name", "work_cases"."master_name"), "work_houres" = COALESCE(UPDATED."work_houres", "work_cases"."work_houres"), "price" = COALESCE(UPDATED."price", "work_cases"."price")\n'
      'FROM ( VALUES ${requests.map((r) => '( ${values.add(r.id)}:int8::int8, ${values.add(r.title)}:text::text, ${values.add(r.carModel)}:text::text, ${values.add(r.serviceId)}:int8::int8, ${values.add(r.photos)}:_text::_text, ${values.add(r.video)}:text::text, ${values.add(r.description)}:text::text, ${values.add(r.masterName)}:text::text, ${values.add(r.workHoures)}:float8::float8, ${values.add(r.price)}:float8::float8 )').join(', ')} )\n'
      'AS UPDATED("id", "title", "car_model", "service_id", "photos", "video", "description", "master_name", "work_houres", "price")\n'
